function export_intensity_stats(smartID,db,stu,syn,save_file)
...
for iLr = 1:length(who)
    ...
    stat = getSoloStats(syn,iFish,cl,flip_class);
...
stat.pairWiseRatioMedianInt.all = median((syn.unchangedAfter.intensity_mp{iFish}-...
    syn.unchangedBefore.intensity_mp{iFish})./...
    syn.unchangedBefore.intensity_mp{iFish});

% get the top 80%:
%     isTop = getTopBandA(syn.unchangedAfter.intensity_mp{iFish},...
%             syn.unchangedBefore.intensity_mp{iFish}, 80);

normB = median(syn.unchangedBefore.intensity_mp{iFish});
stat.pairWiseDiffMedianInt.all = median((syn.unchangedAfter.intensity_mp{iFish}-...
    syn.unchangedBefore.intensity_mp{iFish}))/normB;

% on different sides
[latSideIdx, medSideIdx] = splitPoints(syn.unchangedBefore.xyz_anat_mp{iFish},cl,flip_class);

stat.pairWiseRatioMedianInt.lat = median((syn.unchangedAfter.intensity_mp{iFish}(latSideIdx,:)-...
    syn.unchangedBefore.intensity_mp{iFish}(latSideIdx,:))./...
    syn.unchangedBefore.intensity_mp{iFish}(latSideIdx,:));
...
%     isTop = getTopBandA(unchangedAfter,unchangedBefore, 80);
stat.pairWiseDiffMedianInt.lat = median((unchangedAfter-...
    unchangedBefore))/normB;

stat.pairWiseRatioMedianInt.med = median((syn.unchangedAfter.intensity_mp{iFish}(medSideIdx,:)-...
    syn.unchangedBefore.intensity_mp{iFish}(medSideIdx,:))./...
    syn.unchangedBefore.intensity_mp{iFish}(medSideIdx,:));
...
%     isTop = getTopBandA(unchangedAfter,unchangedBefore, 80);
stat.pairWiseDiffMedianInt.med = median((unchangedAfter-...
    unchangedBefore))/normB;
...
stat.pairWiseRatioMedianIntBlCorr.all = median((syn.unchangedAfter.intensity_mp{iFish}*mult-...
    syn.unchangedBefore.intensity_mp{iFish})./...
    syn.unchangedBefore.intensity_mp{iFish});

normB = median(syn.unchangedBefore.intensity_mp{iFish});
stat.pairWiseDiffMedianIntBlCorr.all = median((syn.unchangedAfter.intensity_mp{iFish}*mult-...
    syn.unchangedBefore.intensity_mp{iFish}))/normB;

% on different sides
[latSideIdx, medSideIdx] = splitPoints(syn.unchangedBefore.xyz_anat_mp{iFish},cl,flip_class);

stat.pairWiseRatioMedianIntBlCorr.lat = median((syn.unchangedAfter.intensity_mp{iFish}(latSideIdx)*mult-...
    syn.unchangedBefore.intensity_mp{iFish}(latSideIdx))./...
    syn.unchangedBefore.intensity_mp{iFish}(latSideIdx));

stat.pairWiseDiffMedianIntBlCorr.lat = median((syn.unchangedAfter.intensity_mp{iFish}(latSideIdx)*mult-...
    syn.unchangedBefore.intensity_mp{iFish}(latSideIdx)))/normB;

stat.pairWiseRatioMedianIntBlCorr.med = median((syn.unchangedAfter.intensity_mp{iFish}(medSideIdx)*mult-...
    syn.unchangedBefore.intensity_mp{iFish}(medSideIdx))./...
    syn.unchangedBefore.intensity_mp{iFish}(medSideIdx));

stat.pairWiseDiffMedianIntBlCorr.med = median((syn.unchangedAfter.intensity_mp{iFish}(medSideIdx)*mult-...
    syn.unchangedBefore.intensity_mp{iFish}(medSideIdx)))/normB;

end

function stat = getSoloStats(syn,iStudy,cl,flip_class)
%% get stats
...
%% pairwise intensity shift
stat = getPairWiseStatSolo(stat,syn,iStudy,cl,flip_class);
stat = getPairWiseStatBleachingCorrectedSolo(stat,syn,iStudy,cl,flip_class,3.86);
